...
const Post = ({ post }) => {
    ...
    const deletePostHandler = async () => {
        try {
            const res = await axios.delete(`http://localhost:8512/api/v1/post/delete/${post?._id}`, { withCredentials: true });
            if (res.data.success) {
        ...
        const bookmarkHandler= async()=>{
            try {
                const res= await axios.get(`http://localhost:8512/api/v1/post/${post?._id}/bookmark`, { withCredentials: true })
                if(res.data.success){
                    toast.success(res.data.message)
                }
            } catch (error) {
    ...
    const commentHandler = async () => {
        try {
            const res = await axios.post(`http://localhost:8512/api/v1/post/${post?._id}/comment`, { text }, {
                headers: {
    ...
    const likeorDislikeHandler = async () => {
        try {
            ...
            const res = await axios.get(`http://localhost:8512/api/v1/post/${post._id}/${action}`, { withCredentials: true });
            if (res.data.success) {
    ...
    return (
        <div className="post-container  w-full max-w-[500px] mx-auto bg-[#0F0F0F] text-gray-300 p-4 rounded-lg shadow-md">
            {/* Header */}
            <div className="flex items-center justify-between mb-4">
                ...
                <Dialog>
                    ...
                    {
                        post?.author?._id!==user?._id &&  <Button variant="ghost" className="cursor-pointer w-fit text-[#ED4956] font-bold hover:bg-[#1F1F1F] hover:text-[#FF6666]">
                            Unfollow
                        </Button>
                    }
    ...
    navigate('/chat'); 
            ...
            <div className="mt-4 flex items-center justify-between">
                ...
                />
                {text && (
                    <span onClick={commentHandler} className="text-[#3BADF8] cursor-pointer hover:text-[#5BC2FF]">
                        Post
                    </span>
                )}
            </div>
